...
#if defined(NDEBUG)
#error "KMI cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uKMI) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight >=  200001 && nHeight <=  300000) {nSubsidy =        4.25 * COIN;}
    else if (nHeight >=  300001 && nHeight <=  400000) {nSubsidy =        4.00 * COIN;}
    else if (nHeight >=  400001 && nHeight <=  500000) {nSubsidy =        3.75 * COIN;}
    else if (nHeight >=  500001 && nHeight <=  600000) {nSubsidy =        3.50 * COIN;}
    else if (nHeight >=  600001 && nHeight <=  700000) {nSubsidy =        3.25 * COIN;}
    else if (nHeight >=  700001 && nHeight <=  800000) {nSubsidy =        3.00 * COIN;}
    else if (nHeight >=  800001 && nHeight <=  900000) {nSubsidy =        2.75 * COIN;}
    else if (nHeight >=  900001 && nHeight <= 1000000) {nSubsidy =        2.50 * COIN;}
    else if (nHeight >= 1000001 && nHeight <= 2500000) {nSubsidy =        2.25 * COIN;}
    else if (nHeight >= 2500001 && nHeight <= 5000000) {nSubsidy =        2.00 * COIN;}
    else if (nHeight >= 5000001 && nHeight <= 7500000) {nSubsidy =        1.75 * COIN;}
    else if (nHeight >= 7500001 && nHeight <=10000000) {nSubsidy =        1.50 * COIN;}
    else if (nHeight >=10000001 && nHeight <=50000000) {nSubsidy =        1.25 * COIN;}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    else if (nHeight >=    5001 && nHeight <=   50000)  {nSubsidy = blockValue * 0.60;}
...
{
    RenameThread("koinmudra-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // KOINMUDRA: We use certain sporks during IBD, so check to see if they are
        ...
        vector<CInv> vInvWait;
        {
            ...
            BOOST_FOREACH (const CInv& inv, pto->vInventoryToSend) {
                ...
                if (inv.type == MSG_TX && !fSendTrickle) {
                    // 1/4 of tx invs kmi to all immediately
                    static uint256 hashSalt;
                    if (hashSalt == 0)
                        hashSalt = GetRandHash();
