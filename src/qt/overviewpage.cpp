...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::KMI)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount kmiAvailableBalance = balance - immatureBalance - nLockedBalance;

    // KMI Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // KMI labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, kmiAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // KMI Available
    bool showKMIAvailable = settingShowAllBalances || kmiAvailableBalance != nTotalBalance;
    bool showWatchOnlyKMIAvailable = showKMIAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showKMIAvailable || showWatchOnlyKMIAvailable);
    ui->labelBalance->setVisible(showKMIAvailable || showWatchOnlyKMIAvailable);
    ui->labelWatchAvailable->setVisible(showKMIAvailable && showWatchOnly);

    // KMI Pending
    bool showKMIPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyKMIPending = showKMIPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showKMIPending || showWatchOnlyKMIPending);
    ui->labelUnconfirmed->setVisible(showKMIPending || showWatchOnlyKMIPending);
    ui->labelWatchPending->setVisible(showKMIPending && showWatchOnly);
    ...
    // KMI Locked
    bool showKMILocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyKMILocked = showKMILocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showKMILocked || showWatchOnlyKMILocked);
    ui->labelLockedBalance->setVisible(showKMILocked || showWatchOnlyKMILocked);
    ui->labelWatchLocked->setVisible(showKMILocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("KMI")
    updateDisplayUnit();
}
...
void OverviewPage::SetLinks()
{
    ...
    ui->labelLinks5->setText("Telegram:");
